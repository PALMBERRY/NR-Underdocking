...
typedef struct DmtxRegion_struct {
   ...
   double		   res_angle;		//临时增加
   DmtxVector2	   res_loc;			//临时增加
} DmtxRegion;
...
{
	uint16_t arrive;			//最高位为assigned，1到7位为正方向的x坐标，8到14位为正方向的y坐标,最高只能保存127以内的坐标值
	uint16_t arrive_;			//最高位为assigned，1到7位为负方向的x坐标，8到14位为负方向的y坐标
	uint8_t visited;			//轮廓信息	//最高位为visited，4到6位为sign<0的位置（右上），1到3位为sign>0的位置（左下）
	uint8_t depart;				//梯度相关信息，最高位为assigned，1到3位为depart
	int16_t grad;				//梯度量
}DmtxImgInfo;
...
/* 初始化DmtxDecode结构体
* img				输入DmtxImage结构体
* last_bottom_angle	上一次识别到的底边角度
* 返回值			初始化后的DmtxDecode结构体指针*/
...
/* DM码粗定位，获取粗定位的DM码中心点位置。
* srcImg			输入图像
* oriLoc			DM码中心点定位结果
* new_pic_cnt	已就绪的图像帧计数地址，当该地址指向的值大于0时则认为处理超时，将退出检索
