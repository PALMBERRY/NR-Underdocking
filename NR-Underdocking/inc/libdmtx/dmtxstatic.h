...
* dec				输入DmtxDecode结构体
* loc0				待检测的坐标点
* 返回值			DmtxPass表示该点为种子点，DmtxPass表示该点非种子点*/
/*static*/ DmtxPassFail MatrixRegionSeekEdge(DmtxDecode *dec, DmtxPixelLoc loc0);

/* 搜索轮廓并定位下边沿与左边沿
* dec				输入DmtxDecode结构体
* reg				输入DmtxRegion结构体
* locBegin			种子点坐标
* LinePoints		下边沿与左边沿的轮廓点集
* 返回值			DmtxPass表示成功，DmtxFail表示失败*/
/*static*/ DmtxPassFail MatrixRegionOrientation3(DmtxDecode *dec, DmtxRegion *reg, DmtxPixelLoc locBegin, dmLinePoints* LinePoints);
...
/*static*/ DmtxPassFail GetPointFlow(DmtxDecode *dec, DmtxPixelLoc loc, DmtxImgInfo* cache);
/*static*/ DmtxPassFail FindStrongestNeighbor(DmtxDecode *dec, DmtxImgInfo* center_cache, DmtxPixelLoc center_loc, int sign, DmtxImgInfo** max_cache, DmtxPixelLoc* max_loc);

/*static*/ DmtxPassFail TrailBlazeContinuous(DmtxDecode *dec, DmtxRegion *reg, DmtxPixelLoc locBegin, int maxDiagonal);
/*static*/ int TrailBlazeGapped(DmtxDecode *dec, DmtxRegion *reg, DmtxBresLine line, int streamDir);
/*static*/ void TrailClear(DmtxDecode *dec, DmtxRegion *reg);

/*static*/ DmtxBestLine FindBestSolidLine_angleLimit(DmtxDecode *dec, DmtxRegion *reg, int step0, int streamDir, int houghAvoid, int is_Avoid);

/*static*/ DmtxBestLine FindBestSolidLine2(DmtxDecode *dec, DmtxRegion *reg, int tripSteps, int sign, int houghAvoid);
...
/*static*/ DmtxScheme GetEncodationScheme(unsigned char cw);
...
/*static*/ DmtxPassFail RsDecode(unsigned char *code, int sizeIdx);
/*static*/ DmtxPassFail RsGenPoly(DmtxByteList *gen, int errorWordCount);
/*static*/ DmtxBoolean RsComputeSyndromes(DmtxByteList * restrict syn, const DmtxByteList * restrict rec, int blockErrorWords);
...
/*static*/ void EncodeNextChunk(DmtxEncodeStream *stream, DmtxScheme scheme, int subScheme, int sizeIdxRequest);
...
/*static*/ void ShiftValueListBy3(DmtxByteList* restrict list, DmtxPassFail *passFail);
...
/*static*/ void Base256OutputChainRemoveFirst(DmtxEncodeStream* restrict stream);
/*static*/ DmtxByte Randomize255State(DmtxByte cwValue, int cwPosition);

/*static*/ extern const int dmtxNeighborNone/* = 8*/;
/*static*/ extern const int dmtxPatternX[]/* = { -1,  0,  1,  1,  1,  0, -1, -1 }*/;
/*static*/ extern const int dmtxPatternY[]/* = { -1, -1, -1,  0,  1,  1,  1,  0 }*/;
/*static*/ extern const DmtxPointFlow dmtxBlankEdge /*= { 0, 0, 0, DmtxUndefined, { -1, -1 } }*/;

/*@ +charint @*/

/*static*/extern int rHvX[] /*=
...
/*static*/extern char *dmtxErrorMessage[] /*= {
   "Unknown error",
