...
void Laser2Line::setLaserData(const PointList or_LaserData)
{
	...
	laser_line.clear();
//	printf("already set laser data its size is %d\n",Laser_Data.size());
}
...
{	
	for(int i=0; i<laser_line.size(); ++i)
	{
		...
		if( laser_line[i].end-laser_line[i].start + 1<MIN_POINT_NUM )	erase_flag = true;

		// check the length of the line segment
		if( point_point_dist(laser_line[i].start,laser_line[i].end)<MIN_LINE_DIST )	erase_flag = true; 

		// check the line density
		if( (laser_line[i].end-laser_line[i].start)/laser_line[i].line_length<LINE_DENSITY)	erase_flag = true; 
		...
		{
			if( i+1<laser_line.size() && point_point_dist(laser_line[i].end,laser_line[i+1].start)<0.03 )
			{
