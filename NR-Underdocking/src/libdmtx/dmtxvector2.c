...
dmtxVector2Norm(DmtxVector2 *v)
{
   ...
   dmtxVector2ScaleBy(v, _rcpdp(mag));
...
{
   return sqrtdp_i(v->X * v->X + v->Y * v->Y);
}
...
dmtxDistanceFromRay2(const DmtxRay2 *r, const DmtxVector2 *q)
{
   ...
   assert(_fabs(1.0 - dmtxVector2Mag(&(r->v))) <= DmtxAlmostZero);
...
#ifdef DEBUG
   /* Assumes that v is a unit vector */
   if(_fabs(1.0 - dmtxVector2Mag(&(r->v))) > DmtxAlmostZero) {
...
dmtxRay2Intersect(DmtxVector2 *point, const DmtxRay2 *p0, const DmtxRay2 *p1)
{
   ...
   if(_fabs(denom) <= DmtxAlmostZero)
      return DmtxFail;

   dmtxVector2Sub(&w, &(p1->p), &(p0->p));
   numer = dmtxVector2Cross(&(p1->v), &w);

   return dmtxPointAlongRay2(point, p0, numer*_rcpdp(denom));
}
