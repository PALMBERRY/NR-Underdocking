...
#define CHKPASS { if(passFail == DmtxFail) return DmtxFail; }
/*static*/ DmtxPassFail
RsDecode(unsigned char *code, int sizeIdx)
{
   int i;
   int blockStride, blockIdx;
...
/*static*/ DmtxBoolean
RsComputeSyndromes(DmtxByteList * restrict syn, const DmtxByteList * restrict rec, int blockErrorWords)
{
   int i, j;
   DmtxPassFail passFail;
