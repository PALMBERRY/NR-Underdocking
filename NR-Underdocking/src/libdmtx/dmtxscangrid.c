...
InitScanGrid(DmtxDecode *dec)
{
   ...
   smallestFeature = dmtxDecodeGetProp(dec, DmtxPropScanGap);
   ...
   grid.xOffset = (grid.xMin + grid.xMax - grid.maxExtent) >> 1;
   grid.yOffset = (grid.yMin + grid.yMax - grid.maxExtent) >> 1;
   ...
   if(count == grid->pixelTotal - 1) {
      ...
      if(count < half) {
		  if(count < quarter)
		  {
			  //从左到右
			  //*depart = 1;
			  loc.X = grid->xCenter + (count - quarter);
		  }
		  else
		  {
			  //从右到左
			  //*depart = 5;
			  loc.X = grid->xCenter + (half - count);
		  }
      ...
      /* vertical portion */
		 ...
		 if(count < quarter)
		 {
			 //从上到下
			 //*depart = 3;
			 loc.Y = grid->yCenter + (count - quarter);
		 }
		 else
		 {
			 //从下到上
			 //*depart = 7;
			 loc.Y = grid->yCenter + (half - count);
		 }
...
*/
/*static*/ int
GetGridCoordinates2(DmtxScanGrid *grid, DmtxPixelLoc *locPtr)
{
	int count, half, quarter;
	DmtxPixelLoc loc;
	...
	*locPtr = loc;
	...
	if (grid->xCenter + grid->xOffset < grid->xMin || grid->xCenter + grid->xOffset > grid->xMax ||
		grid->yCenter + grid->yOffset < grid->yMin || grid->yCenter + grid->yOffset > grid->yMax)
	{
		grid->pixelCount += grid->pixelTotal - 1;
		return DmtxRangeBad;
	}

	if (loc.X < grid->xMin)
	{
		grid->pixelCount += grid->xMin - loc.X;
		loc.X = grid->xMin;
	}
	else if (loc.X > grid->xMax)
	{
		grid->pixelCount += loc.X - grid->xMax;
		loc.X = grid->xMax;
	}
	else if (loc.Y < grid->yMin)
	{
		grid->pixelCount += grid->yMin - loc.Y;
		loc.Y = grid->yMin;
	}
	else if (loc.Y > grid->yMax)
	{
		grid->pixelCount += loc.Y - grid->yMax;
		loc.Y = grid->yMax;
	}
